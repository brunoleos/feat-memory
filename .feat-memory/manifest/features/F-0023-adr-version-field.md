...
contracts:
  api:
    ...
    - src/feat_memory/memory/schemas.py::SEMVER_RE
    - src/feat_memory/memory/propose_adr.py::generate_draft
  tests:
...
acceptance:
  ...
  - {id: A4, pattern: event, trigger: "`feat-memory propose-adr` gera draft", response: "frontmatter inclui `version: <versão atual do pacote>`"}
depends_on: []
decisions: [ADR-0027]
---
