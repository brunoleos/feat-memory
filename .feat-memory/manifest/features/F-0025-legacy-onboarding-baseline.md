...
user_value: >
  ...
  receber 5 erros e conformidade 0.00 — e, se um projeto legado ficou para trás,
  o próprio audit aponta o remédio (re-deployar). O deploy injeta um esqueleto de
...
contracts:
  api:
    - src/feat_memory/deploy.py::_ensure_frontmatter
    - src/feat_memory/deploy.py::_substitute_tokens
    - src/feat_memory/deploy.py::deploy_constitution
    - src/feat_memory/governance/audit.py::run
  data:
    - src/feat_memory/data/templates/AGENTS.frontmatter-skeleton.md
    - src/feat_memory/data/templates/STATE.md
  tests:
...
acceptance:
  ...
  - {id: A6, pattern: state, state: "a AGENTS.md está sem frontmatter (campos ausentes) e o CLI é mais novo que a versão deployada", response: "o audit imprime a remediação dirigida — re-rode `feat-memory deploy` para injetar o esqueleto"}
depends_on: []
decisions: [ADR-0029]
---
...
humana. O audit completa o ciclo (A6): quando um consumidor legado sem frontmatter
roda um CLI mais novo, liga o sintoma (schema 0.00) ao remédio (re-deploy injeta o
esqueleto).
