...
contracts:
  api:
    - src/feat_memory/governance/check_staleness.py::run
    - src/feat_memory/governance/check_staleness.py::staged_warning
...
acceptance:
  ...
  - {id: A3, pattern: state, state: "staging só com paths não-código (.feat-memory/, tests/, docs/, README.md)", response: "exit 0 silencioso"}
