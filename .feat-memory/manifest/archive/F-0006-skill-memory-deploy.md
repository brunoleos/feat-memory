...
acceptance:
  ...
  - id: A2
    ...
    response: >
      executa `feat-memory deploy` e encerra; não pergunta sobre
  ...
  - id: A3
    ...
    response: >
      executa `feat-memory deploy` e em seguida conduz gênese retroativa
      em três fases: ADRs do git log via `feat-memory migrate`, Manifest
  ...
  - id: A4
    ...
    requirement: >
      ...
      sentinelas markdown feat-memory — identidade, restrições e
...
SKILL.md em [src/feat_memory/data/skills/memory-deploy/SKILL.md](src/feat_memory/data/skills/memory-deploy/SKILL.md). Ponto de entrada único para adoção — invoca `feat-memory deploy` (F-0001) na Etapa 2 do fluxo, e `feat-memory migrate` (F-0004) na Fase 3.1 do sub-fluxo legacy.

Princípio operacional central: a skill toca em `decisions/`, `manifest/` e `STATE.md` durante a gênese retroativa, mas nunca no corpo da `AGENTS.md` fora do bloco com sentinelas. O bloco em si é gerenciado pelo `feat-memory deploy` de forma idempotente. Todo conteúdo de projeto na `AGENTS.md` (identidade, restrições, convenções) é autoria humana.
