...
contracts:
  api:
    - src/feat_memory/shared/paths.py::SUPERSEDED_DIR
    - src/feat_memory/memory/indexing.py::gen_superseded_decisions_index
    - src/feat_memory/memory/indexing.py::regenerate_all_indexes
    - src/feat_memory/governance/audit.py::_resolve_active_decision_paths
    - src/feat_memory/governance/audit.py::run_audit
    - src/feat_memory/memory/propose_adr.py::next_adr_number
  tests:
...
acceptance:
  ...
  - {id: A3, pattern: event, trigger: "`feat-memory audit` executa com superseded populado", response: "regenera `decisions/INDEX.md` (sem superseded) e `decisions/superseded/INDEX.md` (só superseded); ambos com mesmas colunas"}
  ...
  - {id: A5, pattern: ubiquitous, requirement: "movimentação de ADR para superseded é manual via `git mv` — não há subcomando dedicado (ADR-0015 continua válida para `feat-memory archive`)"}
depends_on: [F-0011]
decisions: [ADR-0023]
---
