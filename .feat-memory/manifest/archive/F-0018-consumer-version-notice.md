...
contracts:
  api:
    - src/feat_memory/governance/version_check.py::consumer_version_notice
    - src/feat_memory/governance/version_check.py::run
...
acceptance:
  ...
  - {id: A5, pattern: event, trigger: "`feat-memory version-check` invocado", response: "imprime notice ou \"atualizado: vX.Y.Z\"; exit 0 — subcomando standalone para CI"}
  - {id: A6, pattern: ubiquitous, requirement: "exit do `feat-memory audit` NUNCA é alterado — soft sempre, ADR-0008 preservado"}
