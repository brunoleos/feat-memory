...
# ADR-0005 · Versionar `.feat-memory/` no consumidor (v0.1.0)
...
Clonar a tool dentro do projeto consumidor em `.feat-memory/` e versionar o diretório. Setup trivial (um `git clone`), projeto autocontido, atualização via `git pull` ou re-clone manual.
