...
promete `pipx install feat-memory` "quando publicado na PyPI". Falta fechar esse
caminho.
...
`feat_memory`, mas o split F-0017/ADR-0021 moveu os hooks para
`feat_memory/governance/data/hooks/`. Sem `MANIFEST.in` nem `include_package_data`,
...
1. **package-data por subpacote.** Cada subpacote declara seu próprio data:
   `feat_memory` (templates, skills) e `feat_memory.governance` (hooks). É
