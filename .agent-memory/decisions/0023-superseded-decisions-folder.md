...
ADRs com `status: superseded` ficam em `.agent-memory/decisions/superseded/`. **Mecânica idêntica a `manifest/archive/`:**
...
- `check_collisions` continua funcionando sem mudança (ls-tree recursivo de `.agent-memory/decisions` já varre subpastas).

Movimentação atual é **manual via `git mv`** — o subcomando `agent-memory archive` continua reservado a features (ADR-0015 mantida nessa frente). Se a fricção de mover à mão aparecer, evolução natural é `agent-memory archive --decisions` que move ADRs com `status: superseded` E não citadas em `active_decisions`. Sem caso real ainda, YAGNI.
...
- **Automatizar via `agent-memory archive --decisions` já agora**: superfície sem caso real; layout primeiro, automação se justificar.
