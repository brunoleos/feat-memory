...
acceptance:
  ...
  - {id: A3, pattern: event, trigger: "`agent-memory deploy <target> --no-hooks`", response: "target recebe artefatos de memória completos sem hook de governança"}
  ...
  - {id: A5, pattern: state, state: "`agent-memory --help` invocado", response: "subcomandos agrupados em Memória / Governança via grupos argparse"}
