...
acceptance:
  - {id: A1, pattern: event, trigger: "`agent-memory checkpoint --summary '...' [--current ...] [--next ...]` invocado", response: "cria `.agent-memory/checkpoints/YYYY-MM-DD-HHMMSS.md` (schema em ADR-0019) e regera STATE.md"}
  ...
  - {id: A4, pattern: event, trigger: "`agent-memory state-rebuild` invocado", response: "regera STATE.md sem criar novo checkpoint (recovery)"}
  - {id: A5, pattern: event, trigger: "`agent-memory migrate --to=checkpoints` em checkpoints/ vazio", response: "lê STATE.md legado, cria checkpoint inicial com author=migration, preserva Recent legado no body"}
  ...
  - {id: A8, pattern: ubiquitous, requirement: "memory-debrief invoca `agent-memory checkpoint` em vez de reescrever STATE.md — reescritas destrutivas impossíveis por construção"}
