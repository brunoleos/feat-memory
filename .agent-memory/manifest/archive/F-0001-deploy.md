...
A partir de v0.4.0, não há mais merge queue para `AGENTS.md`/`CLAUDE.md`. O bloco com sentinelas resolve sozinho o caso de re-deploy sobre arquivo customizado, e `CLAUDE.md` (que é só `@AGENTS.md`) é deixado quieto se já existe. O diretório `<target>/.agent-memory-deploy/` legado é removido na primeira execução pós-upgrade.
