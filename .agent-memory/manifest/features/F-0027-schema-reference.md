...
user_value: >
  ...
  código-fonte do agent-memory: `agent-memory schema` imprime uma referência de
...
contracts:
  api:
    - src/agent_memory/memory/schema_reference.py::render_schema_reference
    - src/agent_memory/memory/schema_reference.py::add_subparser
    - src/agent_memory/memory/schemas.py::AGENT_REQUIRED
    - src/agent_memory/memory/schemas.py::FEATURE_REQUIRED
  data:
    - docs/SCHEMA-REFERENCE.md
  tests:
    - tests/test_schema_reference.py
acceptance:
  - {id: A1, pattern: event, trigger: "o usuário roda `agent-memory schema`", response: "imprime a referência de schema gerada das constantes de schemas.py"}
...
por `agent-memory schema` (sempre coerente com o CLI instalado) e materializado em
