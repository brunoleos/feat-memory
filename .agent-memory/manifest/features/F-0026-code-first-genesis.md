...
user_value: >
  ...
  justificando. O `agent-memory migrate` detecta testes, UI e entrypoints de
  forma agnóstica de linguagem, então projetos JS/TS/Go/etc. recebem pistas
  ricas em vez de zero.
contracts:
  api:
    - src/agent_memory/memory/migrate.py::detect_entry_points
    - src/agent_memory/memory/migrate.py::detect_test_signals
    - src/agent_memory/memory/migrate.py::detect_ui_signals
