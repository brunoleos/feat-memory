...
Adotar Architecture Decision Records (ADRs) como forma única de registrar decisões arquiteturais não-triviais. Cada ADR é um arquivo markdown imutável em `.feat-memory/decisions/NNNN-slug.md`, com frontmatter YAML estruturado e seções padronizadas: Contexto, Decisão, Consequências, Alternativas rejeitadas.
