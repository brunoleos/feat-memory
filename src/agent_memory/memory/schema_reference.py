...
código-fonte do agent-memory.

Exposto de duas formas, ambas derivadas da mesma função (zero drift):
- `agent-memory schema` — imprime ao vivo; sempre coerente com o CLI instalado.
...
def render_schema_reference() -> str:
    ...
    w("# Referência de schema — artefatos do agent-memory")
    w("")
    w("Gerado de `agent_memory.memory.schemas` (fonte única). Não edite à mão — "
      "rode `agent-memory schema` ou regenere o doc. Campos não listados como "
    ...
    # Feature
    w("## Feature (`.agent-memory/manifest/features/F-NNNN-slug.md`)")
    w("")
    ...
    # Decision / ADR
    w("## Decisão / ADR (`.agent-memory/decisions/NNNN-slug.md`)")
    w("")
