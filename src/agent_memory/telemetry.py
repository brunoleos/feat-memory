...
Append-only JSONL em `.agent-memory/.telemetry.jsonl` (gitignored).
...
Subcomandos:
    agent-memory record EVENT [field=value ...]
    agent-memory log [--since DAYS] [--event NAME] [--json] [--summary]
...
PRIVACY_HEADER = {
    "_": (
        "agent-memory telemetry — local only, never sent over network. "
        "Disable: .agent-memory/.meta.yaml::telemetry_enabled=false"
    ),
}


def _telemetry_path(root: Path) -> Path:
    return root / ".agent-memory" / TELEMETRY_FILENAME
...
def add_log_subparser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser(
        "log",
        help="Lê e agrega .agent-memory/.telemetry.jsonl",
    )
