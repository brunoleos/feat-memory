...
Cada invocação de `memory-debrief` (ou `agent-memory checkpoint` direto)
cria um arquivo imutável em `.agent-memory/checkpoints/YYYY-MM-DD-HHMMSS.md`.
`STATE.md` é regerado a partir dos checkpoints; mesmo schema do legado,
contrato com `memory-bootstrap` preservado (Liskov-safe).

Subcomandos:
    agent-memory checkpoint --summary "..." [--current ...] [--next ...]
                            [--features F-...,F-...] [--decisions ADR-...,...]
                            [--blocked-on "..."] [--author NAME]
    agent-memory state-rebuild
...
def _checkpoints_dir(root: Path) -> Path:
    return root / ".agent-memory" / "checkpoints"


def _state_path(root: Path) -> Path:
    return root / ".agent-memory" / "STATE.md"
...
def add_checkpoint_subparser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser(
        "checkpoint",
        help="Anexa novo checkpoint ao .agent-memory/checkpoints/ e "
    ...
    p.add_argument("--author", default=None,
                   help="identificação do agente (default: agent-memory)")
...
def run_state_rebuild(args: argparse.Namespace) -> int:
    ...
    if not checkpoints:
        print("Nenhum checkpoint encontrado em .agent-memory/checkpoints/.",
              file=sys.stderr)
        print("Rode `agent-memory checkpoint --summary '...'` para criar o primeiro,",
              file=sys.stderr)
