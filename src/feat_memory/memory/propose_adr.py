...
draft pré-preenchido em .feat-memory/decisions/proposals/.
...
Subcomando da CLI: `feat-memory propose-adr`. Os artefatos ficam
em .feat-memory/ no project root, descoberto via git.

Uso:
    feat-memory propose-adr             # HEAD~1..HEAD
    feat-memory propose-adr --base ABC  # contra commit específico
    feat-memory propose-adr --staged    # mudanças staged
    feat-memory propose-adr --force     # gera mesmo sem sinais
    feat-memory propose-adr --prompt    # emite prompt para LLM
"""
...
def generate_draft(stats: dict, files: list[str], messages: list[str],
    ...
    from feat_memory import __version__ as pkg_version
...
> e mova de `.feat-memory/decisions/proposals/` para `.feat-memory/decisions/`.
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser(
        "propose-adr",
        help="Detecta mudanças que merecem ADR e gera draft em .feat-memory/decisions/proposals/",
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    print(f"  3. mv {rel} .feat-memory/decisions/<NNNN>-<slug>.md")
    print("  4. git add .feat-memory/decisions/<NNNN>-<slug>.md")
